...
def ADD_STUDENT(request):
    ...
    if request.method  == 'POST':
        ...
        sport_ids = request.POST.getlist('sport_id')
        session_year_id = request.POST.get('session_year_id')

        # Validate session_year_id
        if not session_year_id or session_year_id == "Select Session":
        ...
        if CustomUser.objects.filter(username=username).exists():
            ...
            # Add multiple sports to the student
            for sport_id in sport_ids:
                sport = Sport.objects.get(id=sport_id)
                student.sport_id.add(sport)
...
def EDIT_STUDENT(request,id):
    try:
        ...
        # Get selected sports for the student
        student_obj = Student.objects.get(id=id)
        selected_sports = student_obj.sport_id.all()
...
def UPDATE_STUDENT(request):
    if request.method == "POST":
        ...
        # Validate student_id and session_year_id
        if not student_id or student_id == "" or session_year_id == "Select Session":
            messages.error(request, "Please provide valid Student and Session Year.")
        ...
        # Clear existing sports and add new selected sports
        student.sport_id.clear()
        for sport_id in sport_ids:
            sport = Sport.objects.get(id=sport_id)
            student.sport_id.add(sport)
...
def DELETE_SPORT(request,id):
    try:
        ...
        messages.error(request, 'Error deleting sport!')
...
def DELETE_SESSION(request,id):
    ...
    try:
        ...
        messages.error(request, 'Error deleting session!')
...
def STAFF_DISAPPROVE_LEAVE(request,id):
    try:
        ...
        messages.error(request, 'Error disapproving leave!')
    return redirect('staff_leave_view')
